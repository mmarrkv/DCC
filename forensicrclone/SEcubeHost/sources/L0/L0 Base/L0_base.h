 ...
 *
 * The file contains all the prototypes of the L0Base library
 */
...
#include <memory>
#ifdef _WIN32
//Windows -> include "Windows.h" only for windows 32 file
#include "Windows.h"
#else
//include for linux
#include <sys/types.h>
...
public:
	...
	//method to add a device to the array
	void AddDevice();
	//read the amount of devices connected
	size_t GetNDevices();
	//method to clear the device array
	...
	//////////////
	//Device GET methods
	se3File		GetDeviceFile();			//returns dev[this->ptr].f
	uint8_t*	GetDeviceHelloMsg();		//returns dev[this->ptr].info.helloMsg
	se3Char*	GetDeviceInfoPath();		//returns dev[this->ptr].info.path
	uint8_t*	GetDeviceInfoSerialNo();	//dev[this->ptr].info.serialno
	bool		GetDeviceOpened();			//dev[this->ptr].opened
	uint8_t		GetDevicePtr();				//returns ptr
	uint8_t*	GetDeviceRequest();			//returns dev[this->ptr].request
	uint8_t*	GetDeviceResponse();		//returns dev[this->ptr].response
	//Iterator GET methods
	uint8_t*	GetDiscoDeviceHelloMsg();	//returns it.deviceInfo.helloMsg
	se3Char*	GetDiscoDevicePath();		//returns it.deviceInfo.path
	uint8_t*	GetDiscoDeviceSerialNo();	//returns it.deviceInfo.serialno
	se3Char*	GetDiscoDriveBuf();			//returns it.driveIt.buf
	size_t		GetDiscoDriveBufLen();		//returns it.driveIt.bufLen
	se3Char*	GetDiscoDrivePath();		//returns it.driveIt.path
	FILE * 		GetDiscoDriveFile();		//returns it.driveIt.fp
	///////////////
	//SET METHODS//
	///////////////
	//buffer allocation/deallocation
	void	AllocateDeviceRequest();				//allocates dev[this->ptr].request
	void	AllocateDeviceResponse();				//allocates dev[this->ptr].response
	void	FreeDeviceRequest();					//free the allocation dev[this->ptr].request
	void	FreeDeviceResponse();					//free the allocation dev[this->ptr].response
	//device SET methods
	void	SetDeviceFile(se3File file);			//sets dev[this->ptr].f = file
	void	SetDeviceOpened(bool opened);			//sets dev[this->ptr].opened = opened
	bool	SetDevicePtr(uint16_t newPtr);			//sets ptr = newPtr
	//iterator SET methods
	void	SetDiscoDeviceStatus(uint16_t status);	//sets it.deviceInfo.status = status
	void	SetDiscoDriveBufLen(size_t bufLen);		//sets it.driveIt.bufLen = bufLen
	void	SetDiscoDriveBufTermination();			//sets it.driveIt.buf[this->it.driveIt.bufLen] = L'\0'
	void	SetDiscoDrivePath(se3Char* path);		//sets it.driveIt.path = path

	void 	SetDiscoDriveFile(FILE* fp);			//set it.driverit.fp = fp
};
