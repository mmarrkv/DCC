        ...
        uint16_t* dataout_len, uint8_t* dataout)
{
    ...
    if (do_update) {
        ...
        if(chacha->mode == MV_CHACHA20POLY1305_DOBAKE_DEC)
        {
            ...
            // security patch for re-inject attack

            //NOTE: ASSUMING SAME-LENGTH SUBSTITUTION + SINGLE OCCURRENCE
            //NOTE: ASSUMING THAT BLOCK SIZE IS LARGE ENOUGH TO STORE AN ENTIRE HTTP HEADER ie token is not split!!!
            // scan data_dec
            //search for access/refresh tokens
            //if access token found - replace by stored access token placeholder
            //else - if refresh token found - replace by stored refresh token placeholder
            if((search_ptr = memstr(data_dec, flashkey_acc_tkn.data,*dataout_len, flashkey_acc_tkn.data_size)))
            {
                memcpy(search_ptr,flashkey_acc_tkn_plchldr.data, flashkey_acc_tkn_plchldr.data_size);
            } else if((search_ptr = memstr(data_dec, flashkey_rfrsh_tkn.data,*dataout_len, flashkey_rfrsh_tkn.data_size)))
            {
                memcpy(search_ptr,flashkey_rfrsh_tkn_plchldr.data, flashkey_rfrsh_tkn_plchldr.data_size);
            }
